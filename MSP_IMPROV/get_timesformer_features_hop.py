...
class MyDataset(Dataset):
    def __init__(self, folder, target_dict):
        ...
        vid_files = [x.replace("UTD", "MSP") for x in vid_files]
        vid_files = [x.replace("wav", "mp4") for x in vid_files]
        vid_files = [x for x in vid_files if ".mp4" in x]
        vid_files = [x for x in vid_files if 'MSP-IMPROV-S10H-M06-S-FM04.mp4' not in x]
    ...
    def __getitem__(self, vid_ind):
        ...
        class_id = self.target[self.vid_files[vid_ind].replace("mp4", "wav").replace("MSP", "UTD")]
...
def get_features(ind):
    ...
    model= TimeSformer(img_size=224, num_classes=4,
                        num_frames=8, attention_type='divided_space_time',
                        pretrained_model='../../TimeSformer/TimeSformer_divST_8x32_224_K600.pyth')    
    model.to(device)
    checkpoint = torch.load(os.path.join('split', 'video_timesformer_pretrained.tar'), map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.to(device)
    model.eval()
    if os.path.exists(os.path.join('split', "features_timesformer_all")) == False:
        os.mkdir(os.path.join('split', "features_timesformer_all"))
    with torch.no_grad():
        for i, data in enumerate(tqdm(train_loader)):
            ...
            for batch_ind in range(bs):
                ...
                np.save(os.path.join('split', "features_timesformer_all", name.replace("mp4", "npy")), features_video[batch_ind])
