...
def get_features(ind):
    ...
    checkpoint = torch.load(os.path.join('..', 'split', 'best_model_wav2vec_ft.tar'), map_location = device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.to(device)
    model.eval()
    if os.path.exists(os.path.join('..', 'split', "features_wav2vec")) == False:
        os.mkdir(os.path.join("..", 'split', "features_wav2vec"))
    with torch.no_grad():
        for i, data in enumerate(tqdm(train_loader)):
            ...
            for vid_ind in range(len(videos)):
                ...
                np.save(os.path.join("..", 'split', "features_wav2vec", name.replace("wav", "npy")), features_video)
