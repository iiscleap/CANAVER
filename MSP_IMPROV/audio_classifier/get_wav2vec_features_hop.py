...
def get_features(ind):
    ...
    checkpoint = torch.load(os.path.join('..', 'split', '1200best_model_wav2vec_ft.tar'), map_location = device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.to(device)
    model.eval()
    if os.path.exists(os.path.join('..', 'split', "features_wav2vec_new")) == False:
        os.mkdir(os.path.join('..', 'split', "features_wav2vec_new"))
    with torch.no_grad():
        for i, data in enumerate(tqdm(train_loader)):
            ...
            for batch_ind in range(bs):
                ...
                np.save(os.path.join("..", 'split', "features_wav2vec_new", name.replace("wav", "npy")), features_audio[batch_ind])
