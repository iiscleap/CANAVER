...
def get_features(ind):
    ...
    model.to(device)
    model.eval()
    if os.path.exists(os.path.join('split', "features_video_GRU")) == False:
        os.mkdir(os.path.join('split', "features_video_GRU"))
    with torch.no_grad():
        for i, data in enumerate(tqdm(train_loader)):
            ...
            for batch_ind in range(bs):
                ...
                np.save(os.path.join('split', "features_video_GRU", name.replace("mp4", "npy")), features_video[batch_ind])
